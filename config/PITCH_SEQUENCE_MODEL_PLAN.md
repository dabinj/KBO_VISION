...
하지만 이 결과만으로 아래를 단정하면 안 된다.

- 투수-포수가 실제로 약점 방향을 더 자주 공략한다
- 내일 경기에서 특정 타자에게 약점 방향으로 던질 것이다

현재 단계에서 말할 수 있는 것은 오직:

`약점 feature가 위치 예측 모델에서 신호를 보인다`

까지다.

즉 `약점 방향 공략` 자체는 아직 별도 검증이 필요한 가설이다.
...
## 20. Guardrail For Match Preview Outputs

README나 시나리오 보드처럼 경기 프리뷰용 출력물을 만들 때는 아래 원칙을 따른다.

### 20.1 What can be used directly
...
- 직전 구종 이후 다음 구종 전이
- 포수별 수신 경향
- 타순 순환과 이닝 흐름에 대한 시나리오

### 20.2 What must not be over-claimed

- `약점이니까 당연히 그 코스로 간다`
- `포수가 약점을 보고 무조건 그 방향을 부른다`
- `타석 결과를 이미 맞힌 것처럼 단정`

### 20.3 Current status of preview boards

현재 README의 내일 경기 보드는:
...
즉 모델이 모든 타석을 자동 추론한 최종 inference 결과가 아니라, 검증된 패턴을 기반으로 구성한 설명용 예시다.

장기 시퀀스 보드의 최신 버전은 단순 문장 나열이 아니라:
